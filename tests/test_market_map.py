...
def test_beat_rate_counts_windows_not_magnitude():
    ...
    assert t.mean_excess > 0     # one huge win still lifts the mean


def test_consensus_ignores_managers_who_lost():
    payload = {"trackers": [
        {"name": "Winner", "mean_excess": 3.0, "stale_days": 20,
         "holdings": [{"symbol": "AAA", "weight": 0.5}]},
        {"name": "Loser", "mean_excess": -3.0, "stale_days": 20,
...
def test_consensus_ignores_a_stale_book():
    ...
    payload = {"trackers": [
        {"name": "Retired", "mean_excess": 9.0,
         "stale_days": trackers.MAX_BOOK_AGE_DAYS + 1,
         "holdings": [{"symbol": "OLD", "weight": 1.0}]},
        {"name": "Active", "mean_excess": 1.0, "stale_days": 20,
...
def test_following_someone_still_filters_through_our_screen():
    ...
    board = {"trackers": [{
        ...
        "mean_excess": 5.0, "beat_rate": 1.0, "stale_days": 10,
...
def test_only_managers_who_beat_the_index_tilt_the_basket():
    board = {"trackers": [
        {"name": "Winner", "mean_excess": 3.0, "stale_days": 10,
         "holdings": [{"symbol": "AAA", "weight": 1.0}]},
        {"name": "Loser", "mean_excess": -3.0, "stale_days": 10,
